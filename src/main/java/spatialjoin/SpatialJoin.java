...
public class SpatialJoin {
	 ...
	 */
	public static boolean spatialJoin(String rectanglesFilePath,
			String queryWindowFilePath, String ouputFilePath, int partitions) {
		SparkConf conf = new SparkConf().setAppName("Spatial Join Module");
		JavaSparkContext sc = new JavaSparkContext(conf);
		boolean result = spatialJoinHelper(rectanglesFilePath,
				queryWindowFilePath, ouputFilePath, partitions, sc);
	...
	private static boolean spatialJoinHelper(String rectanglesFilePath,
			String queryWindowFilePath, String ouputFilePath, int partitions,
			JavaSparkContext sc) {
		try {
			 ...
			 */
			// Typecast Rectangles
			JavaRDD<Rectangle> rectangles = recStr.map(
					new Function<String, Rectangle>() {
						public Rectangle call(String s) {
							...
							case 18:
								...
								r = new Rectangle(nums[2], nums[3], nums[4],
										nums[5]);
								break;
							...
							case 5:
								...
								r = new Rectangle(nums[0], nums[1], nums[2],
										nums[3], nums[4]);
								break;
			...
			final JavaRDD<Rectangle> query = qwStr.map(
					new Function<String, Rectangle>() {
						public Rectangle call(String s) {
							Rectangle r = null;
							Float[] nums = Utils.splitStringToFloat(s, ",");
			...
			// final JavaRDD<Rectangle> queryWindows = qwStr.map(new
						...
						public Boolean call(Rectangle r) {
							...
							for (final Rectangle rxy : quer) {
								if (Settings.D)
									...
									Utils.Log("Checking if [" + r
											+ "] is inside [" + query + "] ");
								flag = rxy.isPointInside(r.getBottomLeft())
										&& rxy.isPointInside(r.getBottomRight())
										&& rxy.isPointInside(r.getTopLeft())
										&& rxy.isPointInside(r.getTopRight());
								if (flag)
									return flag;
							}
							return flag;
